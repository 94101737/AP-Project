...
public class Enemy3 extends AbstractEnemy{
    ...
    public Enemy3(Panel parent,int x, int y, double ps1, double ps2, int direction){
        super( parent, x,  y,  10,  ps1,  ps2,  3,  direction);
    ...
    public int ManhattansDistance(int dir){
        switch (dir){
            case 1:
                return (Math.abs(((Parent.MyVariables.player.Coordiantes.x-Parent.map_offset.x)-(this.Coordiantes.x-Parent.unit))/Parent.unit)
                        +Math.abs(((Parent.MyVariables.player.Coordiantes.y-Parent.map_offset.y)-(this.Coordiantes.y))/Parent.unit));
            case 2:
                return (Math.abs(((Parent.MyVariables.player.Coordiantes.x-Parent.map_offset.x)-(this.Coordiantes.x+Parent.unit))/Parent.unit)
                        +Math.abs(((Parent.MyVariables.player.Coordiantes.y-Parent.map_offset.y)-(this.Coordiantes.y))/Parent.unit));
            case 3:
                return (Math.abs(((Parent.MyVariables.player.Coordiantes.x-Parent.map_offset.x)-(this.Coordiantes.x))/Parent.unit)
                        +Math.abs(((Parent.MyVariables.player.Coordiantes.y-Parent.map_offset.y)-(this.Coordiantes.y-Parent.unit))/Parent.unit));
            case 4:
                return (Math.abs(((Parent.MyVariables.player.Coordiantes.x-Parent.map_offset.x)-(this.Coordiantes.x))/Parent.unit)
                        +Math.abs(((Parent.MyVariables.player.Coordiantes.y-Parent.map_offset.y)-(this.Coordiantes.y+Parent.unit))/Parent.unit));
        }
        ...
        if(this.TenRandomStep == 0){
            ...
            for (int i =0; i< Directions.size();i++){
                switch (Directions.get(i)){
                    case 1:
                        if(ManhattansDistance(1)<=distance){
                            distance = ManhattansDistance(1);
                            if(RequestLeft()){
                                Direction = 1;
                                this.TenRandomStep = 0;
                            }
                        ...
                        break;
                    case 2:
                        if(ManhattansDistance(2)<=distance){
                            distance = ManhattansDistance(2);
                            if(RequestRight()){
                                Direction = 2;
                                this.TenRandomStep = 0;
                            }
                        ...
                        break;
                    case 3:
                        if(ManhattansDistance(3)<=distance){
                            distance = ManhattansDistance(3);
                            if(RequestUp()){
                                Direction = 3;
                                this.TenRandomStep = 0;
                            }
                        ...
                        break;
                    case 4:
                        if(ManhattansDistance(4)<=distance){
                            distance = ManhattansDistance(4);
                            if(RequestDown()){
                                Direction = 4;
                                this.TenRandomStep = 0;
                            }
