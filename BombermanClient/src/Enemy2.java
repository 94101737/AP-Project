...
public class Enemy2 extends AbstractEnemy {
    ...
    public Enemy2(Panel parent,int x, int y, double ps1, double ps2, int direction){
        super( parent, x,  y,  20,  ps1,  ps2,  2,  direction);
    ...
    public int ManhattansDistance(int dir){
        switch (dir){
            case 1:
                return (Math.abs(((Parent.MyVariables.player.Coordiantes.x)-(this.Coordiantes.x-Parent.unit))/Parent.unit)
                        +Math.abs(((Parent.MyVariables.player.Coordiantes.y)-(this.Coordiantes.y))/Parent.unit));
            case 2:
                return (Math.abs(((Parent.MyVariables.player.Coordiantes.x)-(this.Coordiantes.x+Parent.unit))/Parent.unit)
                        +Math.abs(((Parent.MyVariables.player.Coordiantes.y)-(this.Coordiantes.y))/Parent.unit));
            case 3:
                return (Math.abs(((Parent.MyVariables.player.Coordiantes.x)-(this.Coordiantes.x))/Parent.unit)
                        +Math.abs(((Parent.MyVariables.player.Coordiantes.y)-(this.Coordiantes.y-Parent.unit))/Parent.unit));
            case 4:
                return (Math.abs(((Parent.MyVariables.player.Coordiantes.x)-(this.Coordiantes.x))/Parent.unit)
                        +Math.abs(((Parent.MyVariables.player.Coordiantes.y)-(this.Coordiantes.y+Parent.unit))/Parent.unit));
        }
