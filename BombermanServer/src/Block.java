...
public class Block{
    ...
    public void fire(Panel MyPanel , int FiredBlockType, int OwnerOfFire){
        ...
        BlockFire = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                for (int i = 0; i<100;i++){
                    ...
                    synchronized (MyPanel.MyVariables.PlayerList){
                        for (int j=0; j<MyPanel.MyVariables.PlayerList.size();j++){
                            if(IsInFire(new Point(MyPanel.MyVariables.PlayerList.get(j).Coordiantes.x,MyPanel.MyVariables.PlayerList.get(j).Coordiantes.y),MyPanel.player_width,MyPanel.player_height,MyPanel)){
                                MyPanel.Lose(j);
                            }
                        }
                    }
                ...
                if(MyPanel.MyVariables.table[Block.this.BLOCK_COORDINATES.y/MyPanel.unit][Block.this.BLOCK_COORDINATES.x/MyPanel.unit]==1){
                    switch (Block.this.BLOCK_TYPE){
                        case 2:
                            ...
                            MyPanel.MyVariables.PlayerList.get(OwnerOfFire).IncreasePoints(10);
                            break;
                        case 3:
